...
while True:
    ...
    if lmList != []:
        ...
        xlength = hypot(x6-x1,y6-y1)
        xlength2 = hypot(x6-x3,y6-y3)
        xlength3 = hypot(x6-x4,y6-y4)
        xlength4 = hypot(x6-x5,y6-y5)
 ...
 # from numpy we find our length,by converting hand range in terms of volume range ie b/w -63.5 to 0
        ...
        if int(length) < 20 and int(length2) < 20 and int(length3) <20 and int(length4) <20:
            #mcr.command('execute as @e[tag=stare] at @s rotated as @p run teleport @s ~ ~ ~ ~ ~')
            mcr.command('execute as @e[type=armor_stand, tag=stare] at @s run setblock ^ ^2 ^6 dirt')
            ...
            l = mcr.command('execute as @e[type=armor_stand, tag=stare] at @s run tp @s ~ ~1 ~ ~ ~')
            
            #print(l)
        elif int(length) < 20 and int(length2) < 20:
        
            mcr.command('execute as @e[type=armor_stand,tag=stare] at @s rotated ~ 0 run tp @s ^ ^ ^0.2')
            #l = mcr.command('data merge entity @e[type=armor_stand,limit=1,tag=stare] {Pose:{Head:['+ str(int(y1)) +'f,'+ str(int(x1)) +'f,0f]},ShowArms:1}')
            #print()
        elif int(xlength3) < 30 and int(xlength4)< 30:
            mcr.command('execute as @e[type=armor_stand, tag=stare] at @s run setblock ^ ^2 ^3 dirt')
        elif int(xlength) < 45 and int(xlength2)< 45:
            mcr.command('execute as @e[type=armor_stand, tag=stare] at @s run setblock ^ ^2 ^3 air')
        elif int(length3) < 20:
            #l = mcr.command('data merge entity @e[type=armor_stand,limit=1,tag=stare] {Pose:{Head:['+ str(int(y1)) +'f,'+ str(int(x1)) +'f,0f]},ShowArms:1}')
            ...
            l = mcr.command('execute as @e[type=armor_stand, tag=stare] at @s run tp @s ~ ~ ~ ~ ~-5')
            #print(l)
        elif int(length4) < 20:
            #l = mcr.command('data merge entity @e[type=armor_stand,limit=1,tag=stare] {Pose:{Head:['+ str(int(y1)) +'f,'+ str(int(x1)) +'f,0f]},ShowArms:1}')
            #l = mcr.command('data merge entity @e[type=armor_stand,limit=1,tag=stare] {Pose:{RightArm:['+ str(int(180 - int(y1 / 3))) +'f,'+ str(int (180 - (  int(x1 / 3)) + 90)) +'f,0f]},ShowArms:1}')
            ...
            #l = mcr.command('data merge entity @e[type=armor_stand,limit=1,tag=stare] {Pose:{RightArm:['+ str(int(180 - int(y1 / 3))) +'f,'+ str(int (180 - (  int(x1 / 3)) + 90)) +'f,0f]},ShowArms:1}')
            l = mcr.command('execute as @e[type=armor_stand, tag=stare] at @s run tp @s ~ ~ ~ ~4 ~')
